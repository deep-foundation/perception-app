...
interface IEditor {
  linkId?: Id;
  onSave?: (result: { value: string; slateValue: any; }) => any;
...
const Leaf = ({ attributes, children, leaf }) => {
  ...
  // Only objects editor.
export const Wysiwyg = React.memo(function Wysiwyg({
  linkId,
  onSave,
  subscription,
...
// const randomBorderColor = Math.floor(Math.random()*16777215).toString(16);
  ...
  const bg = useChakraColor('deepBg');
  ...
  const update = useCallback(() => {
    const v = vRef.current?.value || '';
    if (linkId && v?.length > 0) deep.value(linkId, v);
    onSave && onSave(v);
  }, [link]);

  let _value = useMemo(() => {
    const v = (typeof(link?.value?.value) === 'string' ? link?.value?.value : typeof(value) === 'string' ? value : '<p></p>').trim() || '<p></p>';
    if (typeof(v) === 'string' && !!v) {
      const sl = htmlToSlate(v);
      level(sl);
      return sl;
    } else return initialValue;
  }, [value, initialValue, link]);
      ...
      >
        ...
        {!!topmenu && <>
          ...
          >
            ...
            >
              <MarkButton icon={<FiBold style={{padding: '0.2rem'}} />} format='bold' />
              <MarkButton icon={<FiItalic style={{padding: '0.2rem'}} />} format="italic" />
              <MarkButton icon={<FiUnderline style={{padding: '0.2rem'}} />} format="underline" />
              <MarkButton icon={<FiCode style={{padding: '0.2rem'}} />} format="code" />
              
              <BlockButton format="heading-one" icon={<TbNumber1 style={{padding: '0.2rem'}} />} />
              <BlockButton format="heading-two" icon={<TbNumber2 style={{padding: '0.2rem'}} />} />
              <BlockButton format="block-quote" icon={<TbQuote style={{padding: '0.2rem'}} />} setColor={() => setColor(random)}/>
              <BlockButton format="numbered-list" icon={<TbListNumbers style={{padding: '0.2rem'}} />} />
              <BlockButton format="bulleted-list" icon={<TbList style={{padding: '0.2rem'}} />} />
              <BlockButton format="left" icon={<CiTextAlignLeft style={{padding: '0.2rem'}} />} />
              <BlockButton format="center" icon={<CiTextAlignCenter style={{padding: '0.2rem'}} />} />
              <BlockButton format="right" icon={<CiTextAlignRight style={{padding: '0.2rem'}} />} />
              <BlockButton format="justify" icon={<CiTextAlignJustify style={{padding: '0.2rem'}} />} />
              {/* <BlockButton format="client-handler" icon={<CiPenpot style={{padding: '0.2rem'}} />} />
              <BlockButton format="code-editor" icon={<TbBrandVscode style={{padding: '0.2rem'}} />} /> */}
            </Flex>
            {!!(onSave || linkId) && <Box>
              <Button w='3em' h='3em' onClick={update}><MdSaveAlt/></Button>
            </Box>}
          </Flex>
        </>}
        {/* <Box > */}
          <Editable
            style={{ 
              ...
              borderColor: borderColorEditor || bg,
...
export const BlockButton = React.memo(({ format, icon, setColor }:{ format: any; icon: any; setColor?: () => any; }) => {
  const editor = useSlate();
  
  return (
    <Button
      w='2em' h='2em'
      variant={isBlockActive(
        editor,
        format,
        TEXT_ALIGN_TYPES.includes(format) ? 'align' : 'type'
      ) ? 'active' : undefined}
...
export const MarkButton = ({ format, icon }) => {
  ...
  return (
    <Button
      w='2em' h='2em'
      variant={isMarkActive(editor, format) ? 'active' : undefined}
      onClick={event => {
        event.preventDefault();
