...
export const Level = memo(function Level({
  ...
  const ref = useRef<any>();
  useEffect(() => {
    if (!!active && active[0] === i && active?.[2] !== 'current') {
...
export const TreeView = memo(function TreeView({
  ...
  const mapped = list.map((l, i) => {
    const Component = !!l?.value ? LevelValue : Level;
    return <Component key={`${l?.link?.id}-${JSON.stringify(l?.query)}-${i}`} levelsRefs={levelsRefs} links={l} link={l?.link} value={l?.link?.value} active={active} setList={setList} i={i} onEnter={onEnter} onescreen={onescreen}/>
  });
  useEffect(() => {
