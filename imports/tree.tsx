...
// import { link } from 'fs';
// import { Editor } from '../perception-imports/imports/editor';
...
// import { ClientHandler } from '@deep-foundation/perception-imports/imports/client-handler';
// import { useAsyncMemo } from "use-async-memo";
// import { useCanByContain } from '@deep-foundation/perception-imports/imports/hooks';
// import { loader, useLoader, useSymbol } from '@deep-foundation/perception-imports/imports/hooks';
...
//   const packages = deep.useMinilinksSubscription({ type_id: deep.idLocal('@deep-foundation/core', 'Package') });
//   const canInsertHere = useCanByContain(link.id);
//   const can = useAsyncMemo<any>(async () => {
//     const AllowAdmin = await deep.can(deep.linkId, deep.linkId, deep.idLocal('@deep-foundation/core', 'AllowAdmin'));
//     const AllowInsertType = await deep.can(link.id, deep.linkId, deep.idLocal('@deep-foundation/core', 'AllowInsertType'));
//     const limiter = {
//       id: { _in: (await deep.select({ subject_id: { _eq: deep.linkId }, action_id: { _eq: deep.idLocal('@deep-foundation/core', 'AllowInsertType') } }, { table: 'can' })).data.map(c => c.object_id) },
//       in: { type_id: deep.idLocal('@deep-foundation/core', 'Contain'), from_id: { _in: packages.map(l => l.id) } },
//       limit: 30,
//     };
//     return {
//       AllowAdmin, AllowInsertType, limiter, data: false,
//     };
//   }, [packages], { limiter: { id: { _in: [1] } } });
//   can.data = can?.AllowAdmin || can?.AllowInsert;
//   const packageIds = useMemo(() => {
//     return (packages || []).map(l => l.id);
//   }, [packages]);
//   if (can?.limiter?.in?.from_id?._in) can.limiter.in.from_id._in = packageIds;
...
//   deep.useMinilinksApply(`tree-${config.scope}-type`, aboutType || { data: [] });
//   deep.useMinilinksApply(`tree-${config.scope}-from`, from || { data: [] });
//   deep.useMinilinksApply(`tree-${config.scope}-to`, to || { data: [] });
...
//   const alert = <>
//   <Alert status={can.data ? 'info' : 'error'}>
//     <AlertIcon />
//     <AlertTitle>You {can.data ? 'can' : 'can`t'} do it.</AlertTitle>
//   </Alert>
//   <Editor
//     value={`await deep.can(deep.linkId, deep.linkId, deep.idLocal('@deep-foundation/core', 'AllowAdmin'))
// // ${can?.AllowAdmin}
// await deep.can(${link.id}, deep.linkId, deep.idLocal('@deep-foundation/core', 'AllowInsertType'))
// // ${can?.AllowInsertType}`}
//     editable={false} readOnly
//   />
// </>;
...
//       search={can?.AllowAdmin ? 'Type' : undefined}
//       query={can?.AllowAdmin ? undefined : can.limiter}
//       mode='modal'
//       where={{ local: true }}
...
//         }}
//       >
//         <Box position='absolute' right={1} top={1}>{!!can.AllowAdmin || !!canInsertHere ? '🔓' : '🔒'}</Box>
...
//   const canDeleteHere = useCanByContain(link.id);
//   const can = useAsyncMemo<any>(async () => {
//     const AllowAdmin = await deep.can(deep.linkId, deep.linkId, deep.idLocal('@deep-foundation/core', 'AllowAdmin'));
//     const AllowDeleteType = await deep.can(link.type_id, deep.linkId, deep.idLocal('@deep-foundation/core', 'AllowDeleteType'));
//     const AllowDelete = await deep.can(link.id, deep.linkId, deep.idLocal('@deep-foundation/core', 'AllowDelete'));
//     return {
//       AllowAdmin, AllowDeleteType, AllowDelete, data: false,
//     };
//   }, [], {});
//   can.data = can?.AllowAdmin || can?.AllowInsert;
...
//       }}
//     >
//       <Box position='absolute' right={1} top={1}>{!!can.data && !!canDeleteHere ? '🔓' : '🔒'}</Box>
